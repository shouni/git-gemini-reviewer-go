...
本ツールでは、プロンプトのテンプレートロジックが内部サービスから分離されています。
プロンプトテンプレートファイル (`.md`) は、プロジェクトルートの **`prompts/` ディレクトリ**に配置されており、Goの `//go:embed` 機能によってアプリケーションに組み込まれています。プロンプトの内容を変更したい場合は、このディレクトリ内の `.md` ファイルを直接修正してください。
...
| :--- | :--- | :--- |
| **`detail`** | **(ルート)/prompts/detail_review_prompt.md** | **コード品質と保守性の向上**を目的とした詳細なレビュー。可読性、重複、命名規則、一般的なベストプラクティスからの逸脱など、広範囲な技術的側面に焦点を当てます。 |
| **`release`** | **(ルート)/prompts/release_review_prompt.md** | **本番リリース可否の判定**を目的としたクリティカルなレビュー。致命的なバグ、セキュリティ脆弱性、サーバーダウンにつながる重大なパフォーマンス問題など、リリースをブロックする問題に限定して指摘します。 |
