...
| **Backlog 連携** | **標準 `net/http`** | Backlog API ($\text{REST API}$) を使用して、生成されたレビュー結果を課題にコメントとして投稿します。 |
...
* **Go:** Go言語のコンパイラとツールチェーンが必要です ($\text{Go 1.18}$以上を推奨)。
...
2.  ### Goコマンドとしてインストール
    ...
    プロジェクトのディレクトリ内で **`go install`** コマンドを実行することで、**実行ファイルが $\text{\$GOPATH/bin}$ (または $\text{\$HOME/go/bin}$) に配置され、システム全体で利用可能**になります。

    ```bash
    go install
    ```

    > **注意:** $\text{go install}$ 後に `git-gemini-reviewer-go: command not found` と表示される場合は、Goのバイナリディレクトリ（通常は $\text{\$HOME/go/bin}$）にパスが通っているか確認してください。
...
Gemini API を利用するために、APIキーを環境変数に設定する必要があります。**Backlog 連携 (`backlog` コマンド)** を使用する場合は、Backlog の情報も設定してください。
...
export BACKLOG_SPACE_URL="https://your-space.backlog.jp" # スペースのURL全体
```
...
**本ツールを実行する際は、事前に $\text{Git}$リポジトリのルートディレクトリに $\text{cd}$ しておく**ことを推奨します。

ツールは、$\text{SSH}$経由でリモートリポジトリにアクセスし、指定されたブランチをフェッチして比較します。
...
レビュー結果を **Backlog の課題にコメントとして投稿**します。このモードは、事前の環境変数設定（**`BACKLOG_API_KEY`**, **`BACKLOG_SPACE_URL`**）が必要です。
...
## 💻 シェル補完の設定 (Shell Completion)

サブコマンドや長いフラグ名（例：`--base-branch`）を**Tabキーで自動補完**できるように設定します。`Cobra`ベースのツールは、このスクリプトを自動生成できます。

### $\text{Bash}$ の場合

以下のコマンドを実行し、スクリプトを生成して $\text{Bash}$ の設定ファイルに追記します。

```bash
# 1. 補完スクリプトを生成し、ホームディレクトリに保存
git-gemini-reviewer-go completion bash > ~/.git-gemini-reviewer-go-completion.bash

# 2. ~/.bashrc または ~/.bash_profile に追記
echo 'source ~/.git-gemini-reviewer-go-completion.bash' >> ~/.bashrc
source ~/.bashrc
```

### $\text{Zsh}$ の場合

`Zsh` の補完システム（`compinit`）が有効になっていることを前提とします。

```bash
# 1. 補完関数ディレクトリに配置
git-gemini-reviewer-go completion zsh > /usr/local/share/zsh/site-functions/_git-gemini-reviewer-go

# 2. シェルを再起動するか、以下を実行して設定を反映
