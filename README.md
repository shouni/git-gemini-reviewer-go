...
了解しました！ご指摘の点を踏まえ、特に**シェル補完**と**CLIツールのユーザー体験**を向上させるための情報を加えた、最終版の$\text{README}$を出力します。
...
Gemini API を利用するために、APIキーを環境変数に設定する必要があります。**Backlog 連携 ($\text{backlog}$ コマンド)** を使用する場合は、Backlog の情報も設定してください。
...
レビュー結果を **Backlog の課題にコメントとして投稿**します。このモードは、**事前の環境変数設定**（$\text{BACKLOG\_API\_KEY}$, $\text{BACKLOG\_SPACE\_URL}$）が必要です。
