...
以下の要件に従い、`--- mark start ---`で囲まれたマークダウンを、**単一のHTMLファイルブロックとして**、完全なHTML5コードを生成してください。**HTMLコード以外のテキスト**（指示、説明、承諾の言葉など）は一切含めないでください。
...
* `##` (h2) で始まる各セクションは、その `h2` タグ自身と、次の `h2`（または `h1`）の手前まで続くすべてのコンテンツ（`<h3>`, `<p>`, `<ul>` など）をグループ化する `<section>` タグで囲むこと。`<h2>` は `<section>` の最初の要素である必要があります。
